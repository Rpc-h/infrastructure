...
declare HOPRD_API_ENDPOINT_3="$rpch_release.hoprd-3.rpch.tech"
declare HOPRD_API_ENDPOINT_4="$rpch_release.hoprd-4.rpch.tech"
declare HOPRD_API_ENDPOINT_5="$rpch_release.hoprd-5.rpch.tech"
...
function getEnvDetails() {
  ...
  local env=$(jq .environments.$HOPR_ENVIRONMENT <<< $config)
  local network_id=$(jq .network_id <<< $env)
  local network=$(jq .networks.$network_id <<< $config)
  hoprAddress=$(jq .token_contract_address <<< $env)
  nftAddress=$(jq .boost_contract_address <<< $env)
  stakeAddress=$(jq .stake_contract_address <<< $env)
  registryAddress=$(jq .network_registry_contract_address <<< $env)
  provider=$(jq .default_provider <<< $network)
}
getEnvDetails
...
declare hoprd_native_addresses=$(jq .native <<< $hoprdAddresses)
declare hoprd_hopr_addresses$(jq .hopr <<< $hoprdAddresses)
